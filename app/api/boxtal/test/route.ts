...
// -------------------------
// ROUTE API TEST BOXTAL
// -------------------------
export async function GET() {
  try {
    // 1️⃣ Récupérer les clés depuis Supabase
    ...
    // 2️⃣ Auth Boxtal avec Basic Auth
    ...
    // 3️⃣ Création commande test livraison domicile
    const testOrder = {
      shipment: {
        fromAddress: {
          type: "BUSINESS",
      ...
      shippingOfferCode: config?.shipping_offer_code_domicile || "DOMICILE-TEST",
      labelType: "PDF_A4",
    };

    const orderRes = await fetch(
      "https://api.boxtal.build/shipping/v3.1/shipping-order",
      {
        method: "POST",
        headers: {
