...
import { MapPin, Loader2, AlertCircle, CheckCircle2, Search, Navigation, Copy, ExternalLink } from 'lucide-react'
import type { ChronopostRelaisPoint } from './ChronopostRelaisWidget'

interface ChronopostRelaisSelectorProps {
  codePostal: string
  ville?: string
...
export default function ChronopostRelaisSelector({
  codePostal,
  ville,
  onSelect,
  selectedPoint
}: ChronopostRelaisSelectorProps) {
  ...
  // Mettre à jour les données manuelles
  useEffect(() => {
    setManualData(prev => ({
      ...prev,
      codePostal: codePostal,
  ...
  const searchPoints = async () => {
    ...
    try {
      ...
      const response = await fetch(`/api/chronopost/relay-points?${params.toString()}`)
      ...
      if (data.success && data.points && data.points.length > 0) {
        ...
        setError(data.error || 'Aucun point relais trouvé automatiquement. Utilisez le formulaire de saisie manuelle ci-dessous.')
        setPoints([])
      }
    } catch (err: any) {
      console.error('Erreur recherche points relais:', err)
      setError('Erreur lors de la recherche. Utilisez le formulaire de saisie manuelle.')
      setPoints([])
    } finally {
      setLoading(false)
    }
  ...
  const handleSelectPoint = (point: ChronopostRelaisPoint) => {
    onSelect(point)
  }
    ...
    <div className="space-y-4">
      ...
      <div className="bg-blue-500/10 border border-blue-500/50 rounded-lg p-4">
        <div className="flex items-start gap-2">
          ...
          <div className="text-sm text-blue-300 flex-1">
            <p className="font-semibold mb-1">Points relais Chronopost</p>
            <p className="text-xs text-blue-400">
              Recherche pour : <strong>{codePostal}</strong> {ville ? ville : ''}
            </p>
            {loading && (
              <p className="text-xs text-yellow-400 mt-2">⏳ Recherche en cours...</p>
            )}
          </div>
        </div>
      </div>
      ...
      <div className="flex items-center gap-3">
        <div className="flex-1">
          <label className="block text-xs text-gray-300 mb-1">Rayon (km)</label>
          <select
            value={searchRadius}
            onChange={(e) => setSearchRadius(parseInt(e.target.value))}
            className="w-full px-3 py-2 bg-noir-900 border border-noir-700 rounded-lg text-white text-sm"
          >
          ...
          <div className="flex items-start gap-2">
            ...
            <div className="flex-1">
              ...
              <a
                ...
                className="inline-flex items-center gap-1 text-yellow-400 hover:text-yellow-300 underline text-xs"
              >
                Rechercher sur chronopost.fr <ExternalLink className="w-3 h-3" />
              </a>
            </div>
          </div>
        </div>
      ...
      {loading && points.length === 0 ? (
        ...
        <div className="text-center py-4 text-gray-400">
          <p className="text-sm">Aucun point relais trouvé automatiquement</p>
        </div>
      ) : null}

      {/* Formulaire de saisie manuelle */}
      {(!points.length || showManualForm) && (
        <div className="bg-noir-800/50 border border-noir-700 rounded-lg p-4">
          <div className="flex items-center justify-between mb-3">
            ...
            {points.length > 0 && (
              ...
              >
                {showManualForm ? 'Masquer' : 'Afficher'}
              </button>
            )}
          </div>
          ...
          {showManualForm && (
            <div className="space-y-3">
              ...
              <div>
                <label className="block text-xs text-gray-300 mb-1">Code (optionnel)</label>
                <input
                  type="text"
              ...
              <div>
                <label className="block text-xs text-gray-300 mb-1">Adresse *</label>
                <input
                  type="text"
          ...
          {!showManualForm && points.length === 0 && (
            <button
